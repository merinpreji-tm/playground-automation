...
export default class Actions {
    ...
    static loadTestData(testData: String) {
        ...
        const fPath = "./test-data/env-test.json";
    ...
    async waitForApiResponse(api, timeoutInMilliSeconds = env.waitFor.APITIMEOUT) {
        console.log(`Waiting for API call: ${api} - start`);
        const before = performance.now()
        await test.step(`Waiting for API call: ${api}`, async () => { })
        try {
            await this.page.waitForResponse(resp => resp.url().includes(api), { timeout: timeoutInMilliSeconds })
        } catch (error) {
            const after = performance.now();
            const timeTaken = (after - before) / 1000;
            console.error(`\n Failed: -- Waited for ${timeTaken.toFixed(2)} seconds for API call : ${api} \n`);
            return;
        }
        const after = performance.now();
        const timeTaken = (after - before) / 1000;
        console.log(`Waiting for API call: ${api} - Finished after ${timeTaken.toFixed(2)} seconds`);
    }
    ...
    async waitForMultipleApiResponse(api) {
        ...
        try {
            ...
            const after = performance.now();
            const timeTaken = (after - before) / 1000;
            console.error(`\n Failed: -- Waited for ${timeTaken.toFixed(2)} seconds for API calls multiple: ${api} \n`);
            return;
        }
        const after = performance.now();
        const timeTaken = (after - before) / 1000;
        console.log(`Waiting for API calls multiple: ${api} - Finished after ${timeTaken.toFixed(2)} seconds`);
    }
    ...
    async waitForPageToLoad(timeToWait = env.waitFor.MAX) {
        await test.step(`Waiting for the page to load`, async () => {
            try{
                await this.page.waitForLoadState('domcontentloaded', { timeout: env.waitFor.LOW });
                await this.page.waitForTimeout(env.waitFor.MIN)
                await this.page.waitForLoadState('networkidle', { timeout: timeToWait });
                await this.page.waitForLoadState('domcontentloaded', { timeout: timeToWait });
                await this.page.waitForTimeout(env.waitFor.MIN)
                await this.page.waitForLoadState('networkidle', { timeout: env.waitFor.HIGH });
            }catch{
                await this.page.waitForLoadState('domcontentloaded', { timeout: timeToWait });
            }
    ...
    async waitForElementToBeAttached(locator: Locator) {
        ...
        const timeTaken = ((after - before) / 1000)
        if (timeTaken > 10) {
            console.warn(`It took ${timeTaken.toFixed(2)} for the element ${locator} to be visible & attached`);
        }
    }
    ...
    async waitForElementToBeEnabled(locator: Locator) {
        try {
            ...
            console.log("\n Met with error wait waiting for the locator -->  ", locator, "\n");
        }
    }
    /**
        * @author arundevtm
    ...
    async typeText(locator: Locator, valueToEnter, friendlyNameOfField?: string) {
        ...
        loggingLine = friendlyNameOfField != undefined ? loggingLine + friendlyNameOfField : loggingLine;
        console.log(loggingLine);
        await test.step(loggingLine, async () => {
    ...
    async clearAndType(locator: Locator, valueToEnter, friendlyNameOfField?: string) {
        await this.waitForElementToBeVisible(locator);
        await locator.clear();
    ...
    async getVisitCategoryByEvaluate(strLocator: string) {
        ...
        const selection = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getPicker().getSelection().data.category_with_all_ancestors, { strLocator });
        return selection;
    }
    ...
    async getSelectionDisplayNameByEvaluate(strLocator: string) {
        await this.page.waitForTimeout(env.waitFor.MIN);
        const Ext: any = undefined;
        const selection = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getSelection().display_name, { strLocator });
    ...
    async getSelectionClientByEvaluate(strLocator: string) {
        await this.page.waitForTimeout(env.waitFor.MIN);
        const Ext: any = undefined;
        const selection = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getSelection().default_general_address.company, { strLocator });
        console.log("selection found as", selection);
        return selection;
    }
    ...
    async getSelectionDataByEvaluate(strLocator: string) {
        ...
        await this.waitForPageToLoad();
        const Ext: any = undefined;
        const selection = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getSelection().data, { strLocator });
        console.log("selection found as", selection);
        return selection;
    }
    ...
    async getSelectionByEvaluate(strLocator: string) {
        await this.page.waitForTimeout(env.waitFor.MIN);
        const Ext: any = undefined;
        const selection = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getSelection(), { strLocator });
        return selection;
    }
    ...
    async getValueByEvaluate(strLocator: string) {
        await this.page.waitForTimeout(env.waitFor.MIN);
        const Ext: any = undefined;
        const value = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getValue(), { strLocator });
        return value
    }
    ...
    async getSelectionInputValueByEvaluate(strLocator: string) {
        await this.page.waitForTimeout(env.waitFor.MIN);
        const Ext: any = undefined;
        const inputValue = await this.page.evaluate(({ strLocator }) => Ext.Component.from(document.querySelector(strLocator)).getInputValue(), { strLocator });
        console.log("input value in getSelectionInputValueByEvaluate :", inputValue);
        return inputValue;
    }
    ...
    async getDynamicLocator(locatorString: string, newValueToReplace: string) {
        const strOfElement = locatorString.replace("valueToBeReplaced", newValueToReplace);
    ...
    async fetchResponse(api) {
        const response = await this.page.waitForResponse((response) => response.url().includes(api), { timeout: env.waitFor.APITIMEOUT });
    ...
    static async checkJSONContainsArrayvalues(json: Object, array: Array<String>) {
        // let jsArray: Array<String> = [];
        // for (const key in json) {
        //     const item = json[key];
        //     jsArray.push(item);
